...
conformance:
    report="${CONFORMANCE_REPORT:-target/m1-15-conformance.md}"; head="$(git rev-parse HEAD)"; tree="$(git write-tree)"; mkdir -p "$(dirname "$report")"; status=PASS; {{_io}} cargo +1.98.0 test -p workflow-adk --test translation --locked || status=FAIL; {{_io}} cargo +1.98.0 test -p workflow-adk --test events --locked || status=FAIL; {{_io}} cargo +1.98.0 test -p workflow-adk --test tool_bridge --locked || status=FAIL; {{_io}} cargo +1.98.0 test -p workflow-runtime --test m1_07_tool_bridge --locked || status=FAIL; {{_io}} cargo +1.98.0 test -p workflow-testkit --test fault_injection --locked || status=FAIL; {{_io}} cargo +1.98.0 test -p workflow-testkit --test code_investigation --locked || status=FAIL; {{_io}} cargo +1.98.0 test -p workflow-adk --test m1_11_execution_checkpoints --locked || status=FAIL; {{_io}} cargo +1.98.0 test -p workflowctl --test m1_10_adk_run --locked || status=FAIL; {{_io}} cargo +1.98.0 test -p workflowctl --test m1_12_destructive_resume --locked || status=FAIL; {{_io}} cargo +1.98.0 test -p workflow-testkit --test m1_15_conformance --locked || status=FAIL; {{_io}} cargo +1.98.0 run -p workflow-testkit --bin m1-15-report --locked -- "$report" "$head" "$tree" "$status"; test "$status" = PASS
