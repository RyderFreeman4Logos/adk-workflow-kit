...
# Network-free M1-15 aggregate: execute every documented selector and bind its result.
conformance:
    report="${CONFORMANCE_REPORT:-target/m1-15-conformance.md}"; head="$(git rev-parse HEAD)"; tree="$(git write-tree)"; mkdir -p "$(dirname "$report")"; status=PASS; evidence="$(mktemp)"; trap 'rm -f "$evidence"' EXIT; run() { selector="$1"; shift; if output="$("$@" 2>&1)"; then result=PASS; else result=FAIL; status=FAIL; fi; printf '%s\n' "$output"; if test "$result" = PASS && ! printf '%s\n' "$output" | grep -qx 'running 1 test'; then result=FAIL; status=FAIL; fi; printf '%s\t%s\n' "$result" "$selector" >> "$evidence"; }; while IFS= read -r selector; do set -- $selector; run "$selector" {{_io}} cargo +1.98.0 test -p "$1" --test "$3" "$4" --locked -- --exact; done < <({{_io}} cargo +1.98.0 run -q -p workflow-testkit --bin m1-15-report --locked -- --selectors); {{_io}} cargo +1.98.0 run -p workflow-testkit --bin m1-15-report --locked -- "$report" "$head" "$tree" "$status" "$evidence"
