...
Production ADK execution (`FencedModel` / `build_profile_agent`) memoizes successful agent node outputs in a durable store at `<workdir>/node-result-cache/`. The store is fail-closed: hash or schema mismatch is a miss, never a silent hit.
